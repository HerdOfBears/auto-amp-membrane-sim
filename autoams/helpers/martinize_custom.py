...
========================================================================\n
""",
    ("-in_dir",   Option(str,                      1,     "./", "directory of  inputted files")),
    ("-out_dir",  Option(str,                      1,     "./", "directory of outputted files")),
        ...
        for resi, resname, bbb, sidechn, ss in zip(resid, self.sequence, bb, sc, self.secstruc):
            ...
            for atype, aname in zip([bbb] + list(scatoms), CoarseGrained.residue_bead_names):
                ...
                if resname in list(self.options['ForceField'].mass_charge.keys()) and counter != 0:
                    ...
                    aname = Q[counter-1] > 0 and 'SCD' or Q[counter-1] < 0 and 'SCD' or aname
...
def main(options):
    ...
    input_file_path = options["-in_dir"].value + options["-f"].value
    inStream = streamTag(input_file_path)
    ...
    for title, atoms, box in frameIterator(inStream):
        ...
        if options["-x"].value:
            ...
            if cgOutPDB is None:
                cg_output_file_name = options["-out_dir"].value + options["-x"].value
                cgOutPDB = open(cg_output_file_name, "w")
    ...
    if options["-n"].value:
        ...
        NDX_output_file_name = options["-out_dir"].value + options["-n"].value
        outNDX = open(NDX_output_file_name, "w")
    ...
    if options["-nmap"].value:
        ...
        atid = 1
        nmap_output_file_name = options["-out_dir"].value + options["-nmap"].value
        outNDX = open(nmap_output_file_name, "w")
    ...
    if options['-o']:
        ...
        for mi in range(len(molecules)):
            ...
            if mol not in moleculeTypes or options['SeparateTop']:
                ...
                # name = options['-f'].value.split('.')[0] + "_" + options["-ss"].value
                ...
                itp_output_file_name = options["-out_dir"].value + moleculeTypes[mol] + ".itp"
                # destination = options["-o"] and open(moleculeTypes[mol]+".itp", 'w') or sys.stdout
                destination = options["-o"] and open(itp_output_file_name, 'w') or sys.stdout
        ...
        # Output stream
        top_output_file_name = options["-out_dir"].value + options["-o"].value
        top  = options["-o"] and open(top_output_file_name, 'w') or sys.stdout
