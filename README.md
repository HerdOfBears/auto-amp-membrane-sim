## autoams
[![Build Status](https://travis-ci.org/uwescience/autoams.svg?branch=master)](https://travis-ci.org/uwescience/autoams)
...
`autoams/autoams.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `autoams.py` in directory called
`autoams`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import autoams as sb` in an
interactive Python session, the classes and functions defined inside of the
`autoams.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the autoams module, the main classes defined at the bottom of
...
Either way, you can create a `autoams/data` folder in which you can
...
(typically, something like our `autoams/tests/test_autoams.py`). Within each
...
have one such test in `autoams/tests/test_autoams.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/autoams`),
and type:
...
will see a message such as:
    ...
    autoams/tests/test_autoams.py .F...
    ...
    autoams/tests/test_autoams.py:49: AssertionError
...
To document `autoams` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/autoams/blob/master/doc/_static/RTD-advanced-conf.png)

 http://autoams.readthedocs.org/en/latest/
...
A `autoams/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/autoams) for the software. This
...
`autoams` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `autoams`, we use the
...
[here](https://github.com/uwescience/autoams/blob/master/scripts/Figure1.ipynb).
...
button on the front page of the "autoams" repository.
...
`autoams/autoams.py` to be called `smallish/smallish.py`

	git mv autoams smallish
	git mv smallish/autoams.py smallish/smallish.py
	git mv smallish/tests/test_autoams.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `autoams` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `autoams` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `autoams` in them. Type
the following to see where all these files are:

	git grep autoams

You can replace `autoams` for `smallish` quickly with:

	git grep -l 'autoams' | xargs sed -i 's/autoams/smallish/g'
...
Edit all the mentions of `autoams` in the `autoams/__init__.py` file, and in
the `autoams/version.py` file as well.
